...
const actualizarProducto = async (req, res) => {
  ...
  try {
    ...
    if (Array.isArray(categoria_ids)) {
      await pool.query('DELETE FROM catalogo_productos_categorias WHERE catalogo_id = $1', [id]);
    ...
    if (Array.isArray(diseno_ids)) {
      await pool.query('DELETE FROM disenos_catalogo_productos WHERE catalogo_id = $1', [id]);
