...
| 52 | Agente Negro | *(pendiente de commit)* | 🔴 **Incidente real en vivo, reportado con captura de pantalla**: `POST /api/pedidos` daba 500 al crear un pedido para un cliente cuyo `origen_venta` es texto libre (ej. "Fb Luisa Rodriguez"). Causa: `clientes.origen_venta` es TEXT en producción real; `pedidos.origen` seguía siendo el enum rígido de 5 valores de 001. El trigger `fn_pedido_set_origen()` falla el cast al copiar uno en otro. Migración `012`: `pedidos.origen` pasa a TEXT, se elimina el enum `origen_venta` (nada más lo usaba). Sin cambios de código JS. | ✅ | ⏳ pendiente de deploy real | Confirmado con `\d clientes`/`\d pedidos`/valores reales en prod que nada en el código asume el enum fijo (`dashboard.js` agrupa dinámicamente) | ⏳ código listo; falta correr la migración y volver a intentar crear el pedido real que falló (cliente `CL0036`, "Fb Luisa Rodriguez") para confirmar que ya no da 500 |
