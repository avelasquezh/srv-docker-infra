...
- **Agente Negro (retoma)** — el dueño pidió continuar "la migración que dejó
  ...
  **Completo hasta Paso 4.** Ver entrada #50 de la sección 8 para el detalle
  completo, incluido un hallazgo crítico propio (aislamiento de "Tamaño" en
  `producto_variables`) que no estaba anticipado en el plan original. Pasos 5-7
  (validar con curl real, deploy, considerar el drop final) quedan para la
  siguiente sesión o para que el dueño confirme el deploy de este código.

- **Agente Negro (incidente en vivo)** — el dueño reportó con captura de pantalla un
  `500` real al crear un pedido nuevo desde el admin (`POST /api/pedidos`). Log real:
  `invalid input value for enum origen_venta: "Fb Luisa Rodriguez"`. Investigado con
  `\d clientes`/`\d pedidos`/valores reales: `clientes.origen_venta` es **TEXT libre**
  en producción (nombres de referidos de Facebook, no una categoría fija) —
  `pedidos.origen` seguía siendo el enum rígido original de 5 valores. El trigger
  `fn_pedido_set_origen()` copia uno en el otro al crear cualquier pedido; rompía para
  cualquier cliente cuyo `origen_venta` no calzara exacto con el enum. **No es dato
  sucio a corregir — es el mismo patrón de deriva de esquema ya visto varias veces**
  (`estado_pedido`, `medios_pago`). Fix: migración `012`, `pedidos.origen` pasa a TEXT
  libre igual que `clientes.origen_venta`. Verificado antes de escribir que nada en
  JS (admin/API) asume un conjunto fijo de valores para `origen` (`dashboard.js`
  agrupa dinámicamente por lo que venga). Sin cambios de código JS — el trigger sigue
  igual, ahora sin necesidad de cast.

  **Cerrado (sesión posterior, vía chat con el dueño, sin acceso directo al
  servidor).** El deploy de `012` falló en producción: `cannot alter type of a
  column used by a view or rule` — la vista viva `v_pedidos_resumen` (no
  rastreada como dependencia en el momento de escribir `012`) depende de
  `pedidos.origen`. `node-pg-migrate` hizo rollback automático, `012` nunca quedó
  registrada en `pgmigrations`, así que el bug original seguía 100% activo. Fix:
  `013_fix_origen_texto_libre_dependencia_vista.sql` — suelta la vista, hace el
  `ALTER`, la recrea idéntica (definición verificada contra producción real con
  `SELECT pg_get_viewdef('v_pedidos_resumen', true)` antes de escribir la
  migración, coincide exacta con `006`, sin sorpresas). **Segundo gotcha real al
  desplegar:** como `012` seguía sin registro en `pgmigrations`, `node-pg-migrate`
  la reintentaba primero en cada corrida y nunca llegaba a `013` — hubo que
  insertar manualmente el registro de `012` en `pgmigrations` (sin ejecutar su
  contenido, ya que `013` hace el trabajo completo) para que `migrate` avanzara.
  Confirmado por el dueño tras esto: pedido creado con éxito para el cliente que
  fallaba antes. **Mismo patrón que la entrada #48** (migración fallida que
  bloquea las siguientes por no quedar registrada) — vale la pena, si vuelve a
  pasar, ir directo al `INSERT` manual en `pgmigrations` en vez de reintentar
  `migrate` a ciegas.
...
| 50 | Agente Negro | `cbd3383` | Plan 7quinquies (`atributos`→`variables`), Pasos 1-4: **Paso 1** verificado contra prod (2 filas en `atributos`, ambas `booleano`; 0 en `atributo_opciones`; 16 en `catalogo_atributos`; el check constraint real permite `booleano`\|`seleccion`, no `lista` como asumía el plan original). **Paso 2** ya estaba resuelto de antes: el backfill original de Fase 1-3 ya había creado `VAR0002`/`VAR0003` (Piel de conejo/Plumón) y sus 16 `producto_variables` — confirmado por consulta antes de correr nada, no se generó backfill nuevo. **Paso 3**: migración `011` actualiza `adicionales` de `vista_catalogo_agente` para leer de `producto_variables`/`variables`/`variable_valores`, filtrando explícitamente `nombre <> 'Tamaño'` — validado con comparación 1:1 sobre todos los productos activos reales, **0 discrepancias**, antes de escribir la migración. **Paso 4**: `AtributoRepository` repuntado al esquema nuevo. 🔴 **Hallazgo crítico propio, no estaba en el plan original:** `producto_variables` guarda en la misma tabla tanto "Tamaño" (dueño: `domains/catalogo/`) como estos atributos booleanos — una traducción literal de `reemplazarAtributosProducto()` habría borrado la configuración de tamaño del producto cada vez que se edita un atributo. Todas las queries del repositorio excluyen explícitamente `nombre <> 'Tamaño'` para evitarlo. También: traducción de vocabulario de tipo (`seleccion`↔`lista`) preservada en ambas direcciones para no romper el contrato que ya conoce el admin. | ✅ | ✅ | Deploy real: migración `011` corrida (`Migrations complete!`) + restart limpio. **Prueba real en prod con usuario admin temporal**: `GET /api/atributos` confirma que "Tamaño" nunca aparece en la lista (solo `VAR0002`/`VAR0003`) — Paso 4 confirmado en ese punto. 🔴 **2do hallazgo real, encontrado por accidente durante la prueba** (no por revisión de código): al probar `PUT /api/atributos/producto/CAT0032` con un ID de atributo inválido (error del propio agente al dejar un placeholder sin resaltar lo suficiente que debía reemplazarse), el `DELETE` corrió pero el `INSERT` falló — **el producto quedó con 0 atributos asignados**, porque el método no era transaccional. Corregido en el mismo commit envolviendo el método en `this.transaction()` (helper ya existente de `pedidos_publicos`, entrada #41) — confirmado con test que simula el mismo fallo (ID inválido → `ROLLBACK` completo, no deja el `DELETE` sin su `INSERT`). Recuperación real de la asignación original de `CAT0032` confirmada manualmente tras el incidente | ✅ **Pasos 1-4 cerrados, con un bug de atomicidad real encontrado y corregido en el proceso — sin ese incidente real (causado por un error de instrucción del propio agente) no se habría encontrado por revisión de código sola** |
| 51 | Agente Negro | `ddaa81f` | Fix del hallazgo de la entrada #50: `reemplazarAtributosProducto()` envuelto en `this.transaction()`. `npm test` → 177/177 (2 tests nuevos: transaccionalidad del caso feliz con `BEGIN`/`COMMIT`, y `ROLLBACK` completo simulando el mismo fallo real que ocurrió en prod). | ✅ | ✅ | Pool falso con `connect()`/`client.query()`/`release()` simulando ambos casos + **repetición real en producción del escenario que rompió antes**: `PUT` con ID inventado → `500` limpio; estado de `CAT0032` idéntico antes y después del intento fallido (`VAR0002`+`VAR0003` en ambos casos, sin alterar nada) | ✅ **bug de atomicidad cerrado por completo: código + deploy + reproducción real del fallo original, confirmando que ya no corrompe datos** |
| 52 | Agente Negro | `768946b` | 🔴 **Incidente real en vivo, reportado con captura de pantalla**: `POST /api/pedidos` daba 500 al crear un pedido para un cliente cuyo `origen_venta` es texto libre (ej. "Fb Luisa Rodriguez"). Causa: `clientes.origen_venta` es TEXT en producción real; `pedidos.origen` seguía siendo el enum rígido de 5 valores de 001. El trigger `fn_pedido_set_origen()` falla el cast al copiar uno en otro. Migración `012`: `pedidos.origen` pasa a TEXT, se elimina el enum `origen_venta` (nada más lo usaba). Sin cambios de código JS. | ✅ | ⏳ ver #53, la 012 falló en el primer intento de deploy | Confirmado con `\d clientes`/`\d pedidos`/valores reales en prod que nada en el código asume el enum fijo (`dashboard.js` agrupa dinámicamente) | ⏳ código listo pero deploy falló en el intento real — ver #53 |
| 53 | Claude (sesión vía chat, sin acceso directo al servidor) | `bb7e369` | Fix del bloqueo real de `012` al desplegar: `cannot alter type of a column used by a view or rule` — `v_pedidos_resumen` (vista no rastreada como dependencia al escribir `012`) depende de `pedidos.origen`. Migración `013_fix_origen_texto_libre_dependencia_vista.sql`: suelta la vista, hace el `ALTER`, la recrea idéntica. Definición de la vista verificada contra producción real con `pg_get_viewdef('v_pedidos_resumen', true)` antes de escribir la migración — coincide exacta con `006`. **Gotcha adicional en el deploy** (mismo patrón que #48): como `012` seguía sin registro en `pgmigrations` tras su rollback, `node-pg-migrate` la reintentaba primero en cada corrida y nunca llegaba a `013` — hubo que insertar manualmente el registro de `012` en `pgmigrations` (sin ejecutar su contenido) para que `migrate` avanzara. | ✅ | ✅ | El dueño corrió cada paso en su servidor y pegó la salida real en el chat (`\d pedidos`, `docker logs lilop-api`, la migración fallando, el `INSERT` manual en `pgmigrations`, `migrate` corriendo limpio) — sin acceso directo de Claude al servidor en ningún punto | ✅ **confirmado por el dueño: pedido creado con éxito para el cliente que fallaba antes (`origen_venta` "Fb Luisa Rodriguez")** |
| 54 | Claude (sesión vía chat) | *(sin commit — cambios hechos directo en n8n/BD, no en el repo)* | Auditoría real del workflow de WhatsApp en n8n (JSON exportado pegado por el dueño): 3 de las 7 tools del `AI Agent` fallaban. Causas encontradas verificando contra la BD real, no asumiendo: (1) `consultar_politica_cambios` tenía la credencial cruzada — apuntaba a la BD `lilop` en vez de `whatsapp_agent`, donde vive `configuraciones_agente`; (2) esa tabla, aunque en la BD correcta, no tenía ningún `GRANT` otorgado a `whatsapp_agent_user` (mismo patrón de tabla creada a mano sin permisos, confirmado con `\dp`) — se corrió `GRANT SELECT ON configuraciones_agente TO whatsapp_agent_user;`; (3) `consultar_categoria`/`consultar_otras_categorias` comparaban nombre de producto/categoría con `=`/`@>` exacto, sensible a mayúsculas — un cliente escribiendo "duvets" en minúscula caía siempre a "no lo manejamos" aunque existiera con mayúscula. El dueño corrigió la credencial y la query directo en la UI de n8n. | ✅ | ✅ | El dueño corrió cada verificación (`\d`, `\dp`, `\l` para confirmar el nombre real de la BD `whatsapp_agent`) y pegó la salida real antes de aplicar cada fix | ✅ **confirmado: las 3 tools funcionan; el bot ya no da error de permisos ni falla por mayúsculas** |
| 55 | Claude (sesión vía chat) | *(sin commit — el prompt vive en n8n, no en el repo)* | Reescritura completa del *system prompt* del nodo `AI Agent`. Encontrado en la auditoría de #54: el prompt prometía productos sin inventario real (duvets, cobijas, "piel de conejo sintética" — 0 productos en `catalogo_productos`, confirmado por query antes de tocar el prompt), incluyendo un guion completo de objeción de venta para un producto que no existe. Cambios: se quitaron esas menciones; se agregó regla de no decir "categoría"/"categorías" al cliente (lenguaje de sistema, no de venta); se agregó regla de no inventar equivalencias cuando `consultar_otras_categorias` no confirma que algo existe; las 4 políticas (tiempos de entrega, garantía/devoluciones, horario, ubicación) dejaron de estar memorizadas como texto fijo y ahora remiten siempre a sus tools correspondientes. | ✅ | ✅ | Verificado contra `catalogo_productos`/`categorias` reales antes de escribir el nuevo texto (0 productos de conejo/peluche, `Duvets`/`Cortinas`/`Protectores`/`Venus` con 0 productos activos) | ✅ **confirmado por el dueño: pegado en el nodo `AI Agent`** |
| 56 | Claude (sesión vía chat) | `1a0ebdb` | La tabla `categorias` (no rastreada en migraciones, verificado con `\d` antes de escribir nada) mezclaba en un solo listado plano líneas de producto reales (Edredones, Sábanas, Combo...) con material (Microfibra, Venus), target (Infantil, Niño, Niña) y diseño (Diseño, Unicolor) — la causa raíz de que el menú de bienvenida de WhatsApp y el prompt ofrecieran cosas sin sentido de venta. Migración `014_categorias_tipo.sql`: agrega columna `tipo` (`linea_producto`/`material`/`target`/`diseno`), backfill de las 16 filas reales. Backend (`CategoriaRepository`/`MaestroService`/`MaestroController`) y admin (`configuracion.html`/`.js`) actualizados para crear/editar con el nuevo campo. `npm test`: 179/179 (2 nuevos). Cache-busting: `configuracion.js?v=` recalculado con `scripts/bump-asset-version.py` (confirmado idempotente: 0 archivos modificados al re-correrlo). | ✅ | ✅ | Migración corrida en prod por el dueño, `SELECT nombre, tipo FROM categorias` confirma la clasificación real aplicada correctamente; `lilop-api` reiniciado (`cd ~/srv/docker/projects/lilop/api && docker compose restart`, confirmado el nuevo PID con `docker ps`) | ✅ **base lista para el menú dinámico de WhatsApp filtrado por `tipo = 'linea_producto'` — ese menú en sí queda pendiente para la siguiente sesión** |

**Pendiente explícito para la siguiente sesión:** construir el nodo/query en n8n que arme el menú de bienvenida de WhatsApp dinámicamente a partir de `categorias WHERE tipo = 'linea_producto' AND activo` + productos activos, en vez del código hardcodeado actual (nodo `Generar menú de opciones`) que todavía ofrece `duvets`/`cobijas` como botones.
