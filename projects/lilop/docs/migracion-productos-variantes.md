...
| 43 | Agente Negro | *(pendiente de commit)* | Cierre real de Fase 5: `domains/catalogo/CatalogoRepository` deja de leer/escribir `catalogo_precios` — `listar()`/`listarPublicoRaw()` leen la variante base de `variantes`, y `upsertPrecio()` escribe contra `variantes` (asegurando `variable_valor` + `producto_variables` la primera vez que un producto usa un tamaño nuevo, mismo criterio de variante base que el trigger de la entrada #42, mismo patrón de SKU que el backfill original). `CatalogoService`/`CatalogoController` sin cambios — el contrato de salida ya era idéntico. `precio <= 0` sigue borrando (ahora la variante) en vez de guardar cero, mismo comportamiento exacto. | ✅ | ⏳ pendiente de deploy/curl real | Pool falso paso a paso (secuencia completa de queries: resolver/crear variable+valor, asegurar permitido, upsert) + 2 tests viejos de `catalogo.test.js` actualizados (asumían el esquema legacy, ya no aplica) — `npm test` 169/169, 47 suites | ⏳ **validado solo con mocks, nunca contra Postgres real — riesgo residual explícito** (ver nota completa en sección 0): antes de cerrar esta entrada hace falta probar en prod (1) cambiar precio de un tamaño existente, (2) agregar un tamaño nuevo a un producto que no lo tenía, (3) poner precio en 0 y confirmar que la talla desaparece |
