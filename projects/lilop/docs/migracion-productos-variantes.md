...
| 43 | Agente Rojo | *(sin commit de código)* | Cierra #42: deploy real de `008` confirmado + re-validación post-corte sobre TODOS los pedidos reales existentes | N/A (ya cubierto en #42) | ✅ | `git pull` + migración `008` aplicada sin error (`CREATE FUNCTION`/`COMMENT`) + restart sin crash. **Re-comparación post-deploy** (no la misma que #42 — esta vez contra `pedidos.valor_venta` ya recalculado por el trigger nuevo en prod real, no una simulación): los 7 pedidos con productos mantienen exactamente el mismo `valor_venta` que antes del corte (`sigue_igual = t` en los 7, sin excepción) — **cero regresión confirmada sobre datos 100% reales**. **Hallazgo aparte, preexistente, no causado por este cambio:** el producto de `PD0051` tiene `nombre = "Combo Edredon + Juego de Sábanas"` (texto libre, sin tilde, no coincide con ningún nombre real del catálogo — los 4 "Combo Edredón..." reales tienen nombres más largos) — el match por texto exacto nunca resolvió el precio de esa línea, ni con el JOIN legacy ni con el nuevo, mismo resultado `NULL` en ambos por igual (por eso el total agregado del pedido no cambió: esa línea siempre aportó 0). Es deuda de datos existente desde antes de este cambio (probablemente un nombre tecleado a mano en vez de elegido del catálogo, o un producto renombrado/descontinuado después de crear el pedido) — no bloquea el cierre de esta entrada, queda anotado para revisión aparte del dueño si le interesa | ✅ **bloqueante real de Fase 6 resuelto y cerrado por completo**: código + deploy + validación end-to-end contra datos reales, antes y después del corte, cero regresión. Con esto, el `DROP` de `catalogo_precios`/`atributos`/`atributo_opciones`/`catalogo_atributos` (Fase 6, sección 7ter #7) ya no tiene ningún bloqueante técnico conocido — sigue pendiente de que el dueño decida hacer el corte final |
