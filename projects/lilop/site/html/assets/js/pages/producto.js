...
const state = {
  ...
  selectedAttributes: new Set(),  /* ids de atributos boolean activados */
  currentImageIdx:    0,
};
...
/* ─── PRECIO CON ATRIBUTOS (recalculable) ────────────────── */
function getBasePrice() {
  const product = state.product;
  if (!product) return 0;
  const talla = state.selectedVariants['Tamaño'];
  if (talla && Array.isArray(product.precios)) {
    const match = product.precios.find(p => p.tamanio === talla);
    if (match) return match.precio;
  }
  return product.price;
}
function getAttributesExtra() {
  const product = state.product;
  if (!product) return 0;
  return (product.attributes || [])
    .filter(a => state.selectedAttributes.has(a.id))
    .reduce((sum, a) => sum + a.sobreprecio, 0);
}
function updatePriceDisplay() {
  const product   = state.product;
  const priceWrap = document.getElementById('productPriceWrap');
  if (!product || !priceWrap) return;
  const extra      = getAttributesExtra();
  const finalPrice = getBasePrice() + extra;
  const hasSale = product.originalPrice && product.originalPrice > product.price;
  const saving  = hasSale
    ? Math.round(((product.originalPrice - product.price) / product.originalPrice) * 100)
    : null;
  priceWrap.innerHTML = `
    <span class="product-info__price">${store.formatPrice(finalPrice)}</span>
...
/* ─── ATRIBUTOS (switches con sobreprecio) ───────────────── */
function renderAttributes(product) {
  ...
  container.innerHTML = attrs.map(a => `
    <label class="attribute-switch">
      <input type="checkbox" class="attribute-switch__input" data-attribute-id="${a.id}" data-sobreprecio="${a.sobreprecio}"/>
      <span class="attribute-switch__track" aria-hidden="true"></span>
      <span class="attribute-switch__label">${a.nombre} <span class="attribute-switch__extra">(+${store.formatPrice(a.sobreprecio)})</span></span>
    </label>
  `).join('');
  container.querySelectorAll('.attribute-switch__input').forEach(input => {
    input.addEventListener('change', () => {
      const id = parseInt(input.dataset.attributeId);
      if (input.checked) state.selectedAttributes.add(id);
      else state.selectedAttributes.delete(id);
      updatePriceDisplay();
      updateWhatsAppLink();
    });
  });
...
function renderVariants(product) {
  ...
  /* Preseleccionar automáticamente el primer tamaño disponible (evita error de obligatoriedad innecesario) */
  const firstTamanioBtn = container.querySelector('.variant-btn[data-variant-group="Tamaño"]');
  firstTamanioBtn?.click();
}
...
function updateWhatsAppLink() {
  ...
  const attrLabels = (product.attributes || [])
    .filter(a => state.selectedAttributes.has(a.id))
    .map(a => a.nombre);
  const variantStr = [
    ...Object.entries(state.selectedVariants).map(([k, v]) => `${k}: ${v}`),
    ...attrLabels,
  ].join(', ');
  ...
  const msg = encodeURIComponent(
    ...
    `Precio: ${store.formatPrice(getBasePrice() + getAttributesExtra())}\n\n` +
    `¿Está disponible?`
  );
...
function initAddToCart() {
  ...
  btn.addEventListener('click', () => {
    ...
    /* Construir string de variante (incluye atributos activados) */
    const attrLabels = (product.attributes || [])
      .filter(a => state.selectedAttributes.has(a.id))
      .map(a => a.nombre);
    const variantStr = [
